        ...
        protected async override Task OnExecuteAsync(object? parameter)
        {
            if (parameter is Uri uri)
            {
                    ...
                    && file != null && !file.IsMissing && !file.IsLoadError)
                {
                    ...
                    {
                        document = new VBACodeDocumentTabViewModel(uri, file.Name, file.Content, false, _showSettingsCommand, _closeToolWindowCommand);
                    ...
                    else
                    {
                        ...
                        {
                            case "md":
                                document = new MarkdownDocumentTabViewModel(uri, file.Name, file.Content, false, _showSettingsCommand, _closeToolWindowCommand);
                                view = new MarkdownEditorControl() { DataContext = document };
                                break;
                            case "rdproj":
                                document = new RubberduckProjectDocumentTabViewModel(uri, ProjectFile.FileName /* TODO put the project name here */, file.Content, false, _showSettingsCommand, _closeToolWindowCommand);
                                view = new SourceCodeEditorControl() { DataContext = document }; // TODO understand json as a different "language"
                                break;
                            default:
                                document = new TextDocumentTabViewModel(uri, file.Name, file.Content, false, _showSettingsCommand, _closeToolWindowCommand);
